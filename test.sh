...
run_benchmark_matlab()
{
    if [ "$1" -nt "$3" -o "$2" -nt "$3" ]; then
        ...
        RUNS=5
        echo "{ { taskset -c ${CORES} sh -c \"for i in \\\$(seq 1 ${RUNS}); do time matlab -batch 'm = dijkstra_matlab(); m.main();'; done } 3>&2 2>&1 1>&3 | grep -v -e "^\$" | tee \"$3\"; } 2>&1"
        { { taskset -c ${CORES} sh -c "for i in \$(seq 1 ${RUNS}); do time matlab -batch 'm = dijkstra_matlab(); m.main();'; done"; } 3>&2 2>&1 1>&3 | grep -v -e "^$" | tee "$3"; } 2>&1 || exit 1
    fi
}

#run_benchmark_octave(script, benchmark, log, cores)
run_benchmark_octave()
{
    if [ "$1" -nt "$3" -o "$2" -nt "$3" ]; then
        if [ "$4" == "all" ]; then
            CORES="1,2,3,4"
        ...
        RUNS=5
        echo "{ { taskset -c ${CORES} sh -c \"for i in \\\$(seq 1 ${RUNS}); do time octave-cli --eval 'm = dijkstra_matlab(); m.main();'; done } 3>&2 2>&1 1>&3 | grep -v -e "^\$" | tee \"$3\"; } 2>&1"
        { { taskset -c ${CORES} sh -c "for i in \$(seq 1 ${RUNS}); do time octave-cli --eval 'm = dijkstra_matlab(); m.main();'; done"; } 3>&2 2>&1 1>&3 | grep -v -e "^$" | tee "$3"; } 2>&1 || exit 1
    fi
}
...
run_benchmark_octave "$BUILD/dijkstra_matlab.m" "$BUILD/dijkstra.txt" "$BUILD/dijkstra_matlab_octave.txt" || exit 1
